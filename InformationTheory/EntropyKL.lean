import InformationTheory.ShannonEntropy.Main
...
2. **Knuth–Skilling derivation** (Appendix C → divergence → entropy on `ProbDist`):
   `KnuthSkilling/Information/*`
3. **Bridges** between representations and between discrete and measure-theoretic KL:
   `InformationTheory/ShannonEntropy/Interface.lean`
   `InformationTheory/ShannonEntropy/MeasureTheoreticBridge.lean`
...
- For "entropy/KL from *all* routes":
  `import InformationTheory.EntropyKL`

## Canonical objects + bridge theorems

The finite distribution types used throughout the project are:

- `ProbVec n` (mathlib `stdSimplex` based), defined in `InformationTheory/Basic.lean`
- `ProbDist n` (foundations-level finite distributions), defined in
  `ProbabilityTheory/Foundations/Distributions/ProbDist.lean`
  and re-exported (as a KS-facing alias) in
  `KnuthSkilling/Information/InformationEntropy.lean`

The main equivalence glue lives in:

- `InformationTheory.probVecEquivProbDist`
- `InformationTheory.shannonEntropy_eq_ks_shannonEntropy`
- `InformationTheory.klDivergenceVec` and `..._eq_klDiv` (measure bridge)
