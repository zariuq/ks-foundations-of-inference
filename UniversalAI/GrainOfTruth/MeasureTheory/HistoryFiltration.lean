...
import UniversalAI.InfiniteHistory
import UniversalAI.GrainOfTruth.FixedPoint
...
namespace UniversalAI.GrainOfTruth.MeasureTheory.HistoryFiltration
...
@[reducible] def sigmaAlgebraUpTo (n : ℕ) : MeasurableSpace Trajectory :=
...
@[reducible] def initialStepMeasure_isProbability (μ : Environment) (h_stoch : isStochastic μ) :
...
@[reducible] def initialStepMeasureWithPolicy_isProbability (μ : Environment) (π : Agent)
    (h_stoch : isStochastic μ) :
...
@[reducible] def initialMeasureIic0WithPolicy_isProbability (μ : Environment) (π : Agent)
    (h_stoch : isStochastic μ) :
...
noncomputable def environmentMeasureWithPolicy (μ : Environment) (π : Agent) (h_stoch : isStochastic μ) :
  ...
  UniversalAI.InfiniteHistory.trajMeasureOf (X := StepFamily)
...
@[reducible] def initialMeasureIic0_isProbability (μ : Environment) (h_stoch : isStochastic μ) :
...
theorem measurable_wrt_filtration_iff (f : Trajectory → ℝ) (t : ℕ) :
  ...
  · -- (→) If measurable w.r.t. ℱ_t = comap (truncate t) ⊤, then depends only on first t steps
    ...
    -- (`sigmaAlgebraUpTo`/`comap` are reducible, so `h_preimage` is defeq to the existential)
  ...
  · -- (←) If depends only on first t steps, then measurable w.r.t. ℱ_t
    ...
    -- (`sigmaAlgebraUpTo`/`comap` are reducible, so the goal is defeq to the existential)
...
end UniversalAI.GrainOfTruth.MeasureTheory.HistoryFiltration
