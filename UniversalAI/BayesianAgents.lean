...
import UniversalAI.BayesianAgents.Core
import Logic.UniversalPrediction
/-!
...
namespace UniversalAI.BayesianAgents
...
def encodeHistory (h : History) : Logic.SolomonoffPrior.BinString :=
...
theorem semimeasure_four_extensions
    (ν : Logic.SolomonoffInduction.Semimeasure)
    (s : Logic.SolomonoffPrior.BinString) :
...
theorem percept_prob_sum_le_one
    (ν : Logic.SolomonoffInduction.Semimeasure)
    (h_enc : Logic.SolomonoffPrior.BinString) :
    (∑' x : Percept,
      if ν h_enc = 0 then 0
...
noncomputable def semimeasureToEnvironment
    (ν : Logic.SolomonoffInduction.Semimeasure) : Environment where
  prob h x :=
...
noncomputable def universalAgent
    (ν : ℕ → Logic.SolomonoffInduction.Semimeasure)
...
-/
noncomputable def AIXI (ν : ℕ → Logic.SolomonoffInduction.Semimeasure)
    (γ : DiscountFactor) (horizon : ℕ) : Agent :=
  ...
  let mixture : BayesianMixture := {
    ...
    weights := Logic.UniversalPrediction.geometricWeight
    weights_le_one := Logic.UniversalPrediction.tsum_geometricWeight_le_one
  }
  bayesOptimalAgent mixture γ horizon
...
noncomputable def AIXIEncode {ι : Type*} [Encodable ι]
    (ν : ι → Logic.SolomonoffInduction.Semimeasure)
    (γ : DiscountFactor) (horizon : ℕ) : Agent :=
  ...
  let weights : ℕ → ENNReal := fun n =>
    match Encodable.decode₂ ι n with
    | some i => Logic.UniversalPrediction.encodeWeight i
    | none => 0
  let mixture : BayesianMixture := {
    envs := envs
    weights := weights
    weights_le_one := by
      ...
      have key : (∑' n : ℕ, weights n) = ∑' i : ι, Logic.UniversalPrediction.encodeWeight i := by
        -- Key insight: weights (encode i) = encodeWeight i since decode₂ (encode i) = some i
        have henc : ∀ i : ι, weights (Encodable.encode i) = Logic.UniversalPrediction.encodeWeight i := by
          intro i
      ...
      rw [key]
      exact Logic.UniversalPrediction.tsum_encodeWeight_le_one
  }
  bayesOptimalAgent mixture γ horizon
...
end UniversalAI.BayesianAgents
