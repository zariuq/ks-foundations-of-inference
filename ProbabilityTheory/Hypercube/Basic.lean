...
namespace ProbabilityTheory.Hypercube
...
and deriving consequences). See `KnuthSkilling/AxiomSystemEquivalence.lean`.
-/
...
3. **BinaryEvidence PLN** (BinaryEvidence counts): different *semantics layer*
   ...
   - Therefore there is **no faithful** point-valued representation `Θ : BinaryEvidence → ℝ`
     (formalized in `Logic.PLN_KS_Bridge`)
...
- foundational carrier: `Logic.EvidenceQuantale` (evidence counts `(n⁺, n⁻)`)
- “no faithful point semantics” gate: `Logic.PLN_KS_Bridge`
- measure-theoretic derivation (with explicit independence assumptions): `Logic.PLNDerivation`
...
See: `Logic.PLNDeduction` and `Logic.PLNFrechetBounds` -/
...
See: Chapter 4 of PLN book, `Logic.EvidenceQuantale` -/
...
- The foundational carrier for PLN-style reasoning is **evidence** `(n⁺, n⁻)`, which has a natural
  partial order with incomparables; see `Logic.EvidenceQuantale` and
  `Logic.PLN_KS_Bridge`.
...
- The canonical “PLN” value space in this repo is **evidence** `(n⁺, n⁻)` (see
  `Logic.EvidenceQuantale`). This carrier has a natural *partial order* with incomparable
  elements, so it does **not** admit a faithful point-valued representation `Θ : BinaryEvidence → ℝ`
  (see `Logic.PLN_KS_Bridge`).
...
`Logic.PLNDerivation` for the explicit measure-theoretic derivation under independence.
...
- `Logic.PLNDeduction` - Core deduction formula (numeric)
- `Logic.PLNDerivation` - Measure-theoretic derivation (explicit independence)
- `Logic.PLNInferenceRules` - Chapter 5-style rules (similarity, MP, etc.)
- `Logic.EvidenceQuantale` - BinaryEvidence counts as a (Heyting/Frame) truth-value carrier
- `Logic.PLN_KS_Bridge` - Totality gate: no faithful `Θ : BinaryEvidence → ℝ`
- `Implementation.MettaVerification` - MeTTa parity (deduction + consistency helpers)
- `Algebra.QuantaleWeakness` - Abstract quantale theory
...
- `Algebra.QuantaleWeakness` - Quantale morphisms + weakness transport (`map_weakness`)
- `ProbabilityTheory.Hypercube.QuantaleSemantics` - Concrete quantale value spaces + morphisms
- `ProbabilityTheory.FreeProbability.Basic` - Noncrossing partitions
- `Logic.EvidenceQuantale` - BinaryEvidence truth values (partial order + tensor + Heyting ops)
- `Logic.PLN_KS_Bridge` - Totality gate: no faithful `Θ : BinaryEvidence → ℝ`
- Future: `Algebra.IntervalQuantale` - Imprecise probability
- Future: `Algebra.FreeQuantale` - Free probability quantale
-/
...
end ProbabilityTheory.Hypercube
