...
import ProbabilityTheory.BayesianNetworks.DiscreteSemantics
...
namespace ProbabilityTheory.BayesianNetworks
...
lemma toValuation_mul (φ ψ : Factor fg) [Mul K] :
    toValuation (φ := Factor.mul (fg := fg) φ ψ) =
      ProbabilityTheory.BayesianNetworks.combine
        (φ := toValuation (φ := φ)) (ψ := toValuation (φ := ψ)) := by
  apply Valuation.ext
  · rfl
  · intro x
    simp [toValuation, Factor.mul, ProbabilityTheory.BayesianNetworks.combine,
      restrict_full]
...
lemma toValuation_sumOut (φ : Factor fg) (v : V)
    ...
    toValuation (φ := Factor.sumOut (φ := φ) v) =
      ProbabilityTheory.BayesianNetworks.sumOut
        (φ := toValuation (φ := φ)) v := by
  classical
  by_cases hv : v ∈ φ.scope
  · apply Valuation.ext
    · simp [toValuation, Factor.sumOut, ProbabilityTheory.BayesianNetworks.sumOut, hv]
    · intro x
      have hassign :
          ∀ val : fg.stateSpace v,
      ...
      calc
        ...
        _ = ∑ val : fg.stateSpace v,
              φ.potential (fun u hu => update x v val u) := by
                ...
                intro val _
                simp [hassign val]
        _ = (ProbabilityTheory.BayesianNetworks.sumOut
              (φ := toValuation (φ := φ)) v).val x := by
                simp [toValuation, ProbabilityTheory.BayesianNetworks.sumOut, hv]
  · apply Valuation.ext
    · simp [toValuation, Factor.sumOut, ProbabilityTheory.BayesianNetworks.sumOut, hv]
    · intro x
      calc
...
lemma toValuation_combineAll (fs : List (Factor fg)) [One K] [Mul K] :
    toValuation (φ := combineAll fs) =
      ProbabilityTheory.BayesianNetworks.combineAll
        (fs.map fun f => toValuation (φ := f)) := by
  classical
  induction fs with
  | nil =>
      simp [combineAll, toValuation_oneFactor,
        ProbabilityTheory.BayesianNetworks.combineAll, oneValuation]
  | cons f fs ih =>
      calc
        ...
        _ =
            ProbabilityTheory.BayesianNetworks.combine
              (φ := toValuation (φ := f))
              (ψ := ProbabilityTheory.BayesianNetworks.combineAll
                (fs.map fun g => toValuation (φ := g))) := by
                simp [ih]
        _ =
            ProbabilityTheory.BayesianNetworks.combineAll
              (toValuation (φ := f) :: fs.map fun g => toValuation (φ := g)) := by
                simp [ProbabilityTheory.BayesianNetworks.combineAll]
...
lemma toValuation_sumOutAll (f : Factor fg) (order : List V)
    ...
    toValuation (φ := sumOutAll f order) =
      ProbabilityTheory.BayesianNetworks.sumOutAll
...
open ProbabilityTheory.BayesianNetworks.BayesianNetwork
open ProbabilityTheory.BayesianNetworks.BayesianNetwork.DiscreteCPT
...
end ProbabilityTheory.BayesianNetworks
