...
/-! ## Probabilistic Semantics (Interfaces)

This file is primarily graph-structural.  However, we also provide *honest* (non-`True`)
interfaces for the probabilistic semantics needed to connect Bayesian networks to Mathlib’s
conditional independence API.

The key design choice here:
- We express conditional independence at the level of **σ-algebras on the joint sample space**
  (generated by coordinate projections), rather than conditional independence of bundled
  “tuples of variables”.

This avoids having to construct large dependent product types just to state the Markov property.
-/

section ProbabilisticInterfaces
...
-- Expose the per-node measurable space as an instance, so we can use `Pi` instances downstream.
noncomputable instance (v : V) : MeasurableSpace (bn.stateSpace v) :=
  bn.measurableSpace v

/-- The joint configuration space `Ω := ∀ v, X_v`. -/
abbrev JointSpace := ∀ v : V, bn.stateSpace v

/-! ### σ-algebras generated by vertex sets -/

/-- The σ-algebra on `JointSpace` generated by the coordinates indexed by a set `S : Set V`. -/
noncomputable def measurableSpaceOfVertices (S : Set V) : MeasurableSpace (bn.JointSpace) :=
  ⨆ v : S, MeasurableSpace.comap (fun ω : bn.JointSpace => ω v) (by infer_instance)

theorem measurableSpaceOfVertices_le (S : Set V) :
    bn.measurableSpaceOfVertices S ≤ (by infer_instance : MeasurableSpace bn.JointSpace) := by
  classical
  -- Each coordinate projection is measurable in the ambient `Pi` measurable space, and `iSup` preserves `≤`.
  refine iSup_le ?_
  intro v
  have hv : Measurable (fun ω : bn.JointSpace => ω v) := by
    -- `measurable_pi_apply` is found by `fun_prop`.
    fun_prop
  exact Measurable.comap_le hv

/-! ### Local Markov Property (stated using Mathlib conditional independence) -/

/-- The “other” (non-descendant) vertices used in the local Markov property:
non-descendants excluding parents and the node itself. -/
def nonDescendantsExceptParentsAndSelf (v : V) : Set V :=
  { u | u ∉ bn.graph.descendants v } \ (bn.graph.parents v ∪ {v})

/-- The local Markov property at a node `v`, with respect to a joint measure `μ` on `JointSpace`.

This is the statement `X_v ⫫ ND(v) \\ (Pa(v) ∪ {v}) | Pa(v)`, expressed as conditional
independence of σ-algebras generated by coordinate projections. -/
def LocalMarkovCondition {V : Type*} [Fintype V]
    (bn : BayesianNetwork V) [∀ v : V, StandardBorelSpace (bn.stateSpace v)]
    (μ : Measure bn.JointSpace) [IsFiniteMeasure μ] (v : V) : Prop :=
...
/-- A BN satisfies the local Markov property if it holds for every node. -/
class HasLocalMarkovProperty {V : Type*} [Fintype V]
    (bn : BayesianNetwork V) [∀ v : V, StandardBorelSpace (bn.stateSpace v)]
    (μ : Measure bn.JointSpace) [IsFiniteMeasure μ] : Prop where
  /-- The local Markov condition holds for each node. -/
  markov_condition : ∀ v : V, LocalMarkovCondition bn μ v

/-! ### Factorization (still WIP)

To connect the DAG structure to an explicit joint measure `μ`, we will eventually:
...
/-! ## Conditional Independence / Factorization (See interfaces above)

For the local Markov property and conditional independence interfaces, see the
`ProbabilisticInterfaces` section earlier in this file.  We keep the core BN file
graph-first, and treat probabilistic semantics as an additional layer. -/
