...
import Logic.PLNDeduction
import Logic.EvidenceClass
import Logic.PLNWeightTV
import Algebra.QuantaleWeakness
...
namespace Logic.EvidenceQuantale

open scoped ENNReal
open Logic.PLNDeduction
...
noncomputable instance : Logic.EvidenceClass.EvidenceType BinaryEvidence where
...
noncomputable instance : CompleteLattice BinaryEvidence where
  ...
  -- In Lean 4.31, `CompleteSemilatticeSup`/`CompleteSemilatticeInf` expose a single field each
  -- (`isLUB_sSup` / `isGLB_sInf`) rather than the four `le_sSup`/`sSup_le`/`sInf_le`/`le_sInf`
  -- fields; we package the same proofs as `IsLUB`/`IsGLB` witnesses.
  sSup := evidenceSSup
  sInf := evidenceSInf
  isLUB_sSup := fun S => by
    constructor
    · -- `evidenceSSup S` is an upper bound of `S`.
      intro x hx
    ...
    · -- `evidenceSSup S` is below any upper bound `x`.
      intro x h
      simp only [evidenceSSup, le_def]
      refine ⟨?_, ?_⟩
      · apply sSup_le
  ...
  isGLB_sInf := fun S => by
    constructor
    · -- `evidenceSInf S` is a lower bound of `S`.
      intro x hx
    ...
    · -- `evidenceSInf S` is above any lower bound `x`.
      intro x h
      simp only [evidenceSInf, le_def]
      refine ⟨?_, ?_⟩
      · apply le_sInf
...
instance : Algebra.QuantaleWeakness.IsCommQuantale BinaryEvidence where
...
noncomputable def toWTV (κ : ℝ≥0∞) (e : BinaryEvidence) : WTV where
  ...
  weight_nonneg := by
    ...
    · -- Main case: use the `c/(1-c)` branch.
      exact Logic.PLNWeightTV.WTV.c2w_nonneg _ (by
      ...
      · -- Main case: all totals nonzero
        ...
        calc x.pos * y.pos + x.neg * y.neg
            ≤ x.pos * y.pos + x.neg * y.neg + (x.pos * y.neg + x.neg * y.pos) := by
              ...
              exact add_nonneg zero_le zero_le
...
noncomputable instance :
    Logic.EvidenceClass.MetaLearnable
      Logic.EvidenceClass.BinaryContext
