...
From Appendix C (Variational/Main.lean), we have the general entropy form:
```
...
namespace KnuthSkilling.Information.Divergence
...
framework. From `Variational/Main.lean`:
...
theorem atomDivergence_eq_entropyForm (w u : ℝ) (hw : 0 < w) (hu : 0 < u) :
    atomDivergence w u =
      Variational.entropyForm u (-log u) 1 w := by
  unfold atomDivergence Variational.entropyForm
  have hu_ne : u ≠ 0 := ne_of_gt hu
  have hw_ne : w ≠ 0 := ne_of_gt hw
  rw [log_div hw_ne hu_ne]
  ring
...
This section formalizes this connection using `InformationTheory.shannonEntropy`
...
This uses `InformationTheory.shannonEntropy` and `ProbVec n`. -/
theorem shannonEntropy_probvec_eq_logn_sub_divergence {n : ℕ} (hn : 0 < n)
    (p : InformationTheory.ProbVec n) :
    InformationTheory.shannonEntropy p =
      log (n : ℝ) - divergence p.1 (fun _ => 1 / (n : ℝ)) := by
  rw [InformationTheory.shannonEntropy]
...
end KnuthSkilling.Information.Divergence
