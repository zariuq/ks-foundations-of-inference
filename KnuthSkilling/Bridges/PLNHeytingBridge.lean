/-
# PLN BinaryEvidence as Heyting K&S Instance

## The Key Insight

PLN BinaryEvidence is NOT Boolean - it has genuinely incomparable elements.
For example: (3⁺, 2⁻) vs (2⁺, 3⁻) - neither dominates the other.

This means:
1. BinaryEvidence is a distributive lattice (PlausibilitySpace level K&S applies)
2. BinaryEvidence is NOT a ComplementedLattice / not Boolean
...
import Logic.EvidenceQuantale
import KnuthSkilling.Core.HeytingValuation
import KnuthSkilling.Core.HeytingBounds
import KnuthSkilling.Core.HeytingIntervalRepresentation

namespace KnuthSkilling.PLNBridge

open Logic.EvidenceQuantale
open KnuthSkilling.Heyting

/-! ## BinaryEvidence Lattice Has Incomparable Elements -/

/-- BinaryEvidence (3, 2): more positive than negative support -/
def evidence_3_2 : BinaryEvidence := ⟨3, 2⟩

/-- BinaryEvidence (2, 3): more negative than positive support -/
def evidence_2_3 : BinaryEvidence := ⟨2, 3⟩

/-- (3, 2) ≤ (2, 3) is false because 3 > 2 in the positive component. -/
theorem not_3_2_le_2_3 : ¬(evidence_3_2 ≤ evidence_2_3) := by
  intro h
  simp only [evidence_3_2, evidence_2_3, BinaryEvidence.le_def] at h
...
/-- Key theorem: (3, 2) and (2, 3) are incomparable in the BinaryEvidence lattice. -/
theorem evidence_incomparable :
    ¬(evidence_3_2 ≤ evidence_2_3) ∧ ¬(evidence_2_3 ≤ evidence_3_2) :=
  ⟨not_3_2_le_2_3, not_2_3_le_3_2⟩

/-- BinaryEvidence is NOT totally ordered: there exist incomparable elements. -/
theorem evidence_not_total_order : ¬(∀ e₁ e₂ : BinaryEvidence, e₁ ≤ e₂ ∨ e₂ ≤ e₁) := by
  push_neg
  exact ⟨evidence_3_2, evidence_2_3, not_3_2_le_2_3, not_2_3_le_3_2⟩

/-- BinaryEvidence satisfies the Incomparable predicate. -/
theorem evidence_has_incomparable :
    ∃ e₁ e₂ : BinaryEvidence, Incomparable e₁ e₂ := by
  use evidence_3_2, evidence_2_3
  exact evidence_incomparable

/-! ## Why 2D BinaryEvidence is the Right Representation
...
The real point is:
1. BinaryEvidence is NOT Boolean (incomparable elements exist)
...
PLN BinaryEvidence is an example of a non-Boolean lattice where:
...
/-- Summary theorem: PLN BinaryEvidence has incomparable elements,
...
but because:
1. BinaryEvidence has incomparable elements (proven above)
2. The 2D carrier preserves this structure
3. Collapsing to 1D loses information
-/

end KnuthSkilling.PLNBridge
