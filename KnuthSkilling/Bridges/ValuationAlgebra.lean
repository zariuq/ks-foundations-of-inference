...
import ProbabilityTheory.BayesianNetworks.ValuationAlgebra
import ProbabilityTheory.BayesianNetworks.FactorGraph
import ProbabilityTheory.BayesianNetworks.VariableElimination
import ProbabilityTheory.BayesianNetworks.ValuationBridge
...
namespace KnuthSkilling.Bridges.ValuationAlgebra
...
def regrade (rep : KnuthSkilling.AdditiveOrderIsoRep α op) :
    α → ℝ :=
  match rep with
  | ⟨Θ, _⟩ => fun x => Θ x

@[simp] theorem regrade_op
    (rep : KnuthSkilling.AdditiveOrderIsoRep α op)
    (x y : α) :
...
def regradeValuation {V : Type*} {β : V → Type*}
    (rep : KnuthSkilling.AdditiveOrderIsoRep α op)
    (φ : ProbabilityTheory.BayesianNetworks.Valuation V β α) :
    ProbabilityTheory.BayesianNetworks.Valuation V β ℝ :=
  { scope := φ.scope
...
def regradeFactorGraph {V : Type*}
    (rep : KnuthSkilling.AdditiveOrderIsoRep α op)
    (fg : ProbabilityTheory.BayesianNetworks.FactorGraph V α) :
    ProbabilityTheory.BayesianNetworks.FactorGraph V ℝ :=
...
noncomputable def weightOfConstraintsKS {V : Type*}
  ...
  by
    ...
    exact
      ProbabilityTheory.BayesianNetworks.VariableElimination.weightOfConstraints
        (fg := fg') constraints
...
open ProbabilityTheory.BayesianNetworks
open ProbabilityTheory.BayesianNetworks.ValuationBridge

variable {α : Type*} [LinearOrder α] {op : α → α → α}

variable {V : Type*} [DecidableEq V]
variable (rep : KnuthSkilling.AdditiveOrderIsoRep α op)
...
noncomputable def ksVE
    ...
    [∀ v, Fintype (fg.stateSpace v)]
    [Fintype fg.factors] :
    ProbabilityTheory.BayesianNetworks.Valuation V (fun v => fg.stateSpace v) ℝ :=
  combineAll (V := V) (β := fun v => fg.stateSpace v) (K := ℝ)
    (eliminateVars (V := V) (β := fun v => fg.stateSpace v) (K := ℝ)
      (toValuations (fg := regradeFactorGraph (rep := rep) fg)
        (VariableElimination.factorsOfGraph (fg := regradeFactorGraph (rep := rep) fg))) order)
...
end KnuthSkilling.Bridges.ValuationAlgebra
