...
See also:
- `KnuthSkilling/Counterexamples/VariationalNonsmooth.lean` for a
  counterexample showing that “variational optimum ⇒ differentiable” is false in general.
- `KnuthSkilling/Counterexamples/CauchyPathology.lean` for an explicit
...
- `KnuthSkilling/ShoreJohnson/KLDerivation.lean` for the Shore-Johnson
  alternative route to the same multiplicative Cauchy/log conclusion (used to reach KL).
- `KnuthSkilling/Bridges/ShoreJohnsonVariationalBridge.lean` for an
...
namespace KnuthSkilling.Variational
...
(system independence) in `KnuthSkilling/Theorem.lean`,
...
- `KnuthSkilling.Information.Divergence` (K&S Section 6)
- `KnuthSkilling.Information.InformationEntropy` (K&S Section 8)
-/
