import KnuthSkilling.Additive.Proofs.GridInduction.Core.Induction.DeltaShift
...
- In the global `B = ∅` regime, `ChooseδBaseAdmissible` is equivalent to the older strict-gap
  ...
  `KnuthSkilling/Additive/Proofs/GridInduction/Core/Induction/Goertzel.lean`.
...
See:
- `KnuthSkilling/Additive/Counterexamples/ZQuantizedBEmpty.lean`
- `KnuthSkilling/Additive/Counterexamples/ZQuantizedBNonempty.lean`
-/
...
`KnuthSkilling/Additive/Proofs/GridInduction/Core/Induction/Goertzel.lean`, but for the
...
`KnuthSkilling.Additive.flip_implies_baseIndexed_B_witness` in
`KnuthSkilling/Additive/Proofs/GridInduction/Core/Induction/Goertzel.lean`.
...
this cannot happen:
`KnuthSkilling/Additive/Proofs/GridInduction/Core/Induction/Goertzel.lean` proves
    ...
    · -- Global B ≠ ∅: strict A/B/C separation forces μy to sit on an exact d^n boundary.
      ...
      · have hKm1_pos : 0 < K - 1 := by omega
        ...
        have : R.Θ_grid ⟨mu F r_old_x, mu_mem_kGrid F r_old_x⟩ -
          ...
          linarith
        simpa [δ] using this
    · -- Global B = ∅: completed via LargeRegimeSeparationStrict.A
  ...
  · -- External case: no B-witness for d^Δ specifically.
    ...
    · -- Global B ≠ ∅: strict A/B/C separation forces μx to sit on an exact d^n boundary.
      ...
      · have hKm1_pos : 0 < K - 1 := by omega
        ...
        simpa [δ] using this
    · -- Global B = ∅: completed via LargeRegimeSeparationStrict.C
      have hB_empty : ∀ r u, 0 < u → r ∉ extensionSetB F d u := by
        intro r u hu hrB
        exact hB_global ⟨r, u, hu, hrB⟩
