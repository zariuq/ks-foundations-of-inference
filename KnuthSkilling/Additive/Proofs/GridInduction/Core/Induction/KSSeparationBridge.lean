...
- The remaining hard content is showing that `KSSeparation` forces commutativity; this is now
  proven (complete) in `.../Additive/Axioms/SandwichSeparation.lean` and re-exported via the theorem
  `Core.op_comm_of_KSSeparation`.

This file provides a tiny convenience wrapper: using
`Additive.Proofs.GridInduction.Core.separationImpliesCommutative_of_KSSeparation`, the existing extension lemma
