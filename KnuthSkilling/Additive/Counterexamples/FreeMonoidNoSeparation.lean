...
import KnuthSkilling.Additive.Counterexamples.RankObstructionLinear
...
noncomputable def op (x y : Word) : Word :=
  ⟨toLex (len x + len y, letters x ++ letters y), by
    ...
    calc len x + len y
        = (letters x).length + (letters y).length := by simp [hx, hy]
      _ = (letters x ++ letters y).length := by simp⟩
...
private theorem len_iterate_op (x : Word) :
  ...
  | succ n ih =>
    ...
    calc
      ...
      _ = (n + 1) * len x + len x := by simp [Nat.add_comm]
      _ = (n + 2) * len x := by
            -- `(n+2) * L = (n+1) * L + L`
...
@[simp] theorem len_KS_op (x y : Word) :
    (KnuthSkillingAlgebraBase.op x y).len = x.len + y.len := by
  rfl
...
theorem op_not_comm : ∃ x y : Word, op x y ≠ op y x := by
  ...
  intro hEq
  have : letters (op w0 w1) = letters (op w1 w0) := by simp [hEq]
      ...
      rk_iterate := by
        ...
        | succ n ih =>
          ...
          calc
            ...
            _ = n • len x + len x := by simp [Nat.add_comm]
            _ = (n + 1) • len x := by
