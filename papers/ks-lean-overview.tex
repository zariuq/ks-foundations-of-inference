...
\author{Codex 5.2, Claude 4.5, Zar Goertzel\thanks{This document was drafted collaboratively by humans and AI systems (GPT/Codex, Claude). While formal claims are machine-checked in Lean 4.28.0, prose descriptions may contain human or AI hallucinations. Caveat lector.}}
\date{\today}
...
All paths below are relative to \texttt{KnuthSkilling/}.
...
\textbf{File}: \texttt{KnuthSkilling/Core/Basic.lean}
...
\textbf{Lines 179--187, Core/Basic.lean}
...
\textbf{Lines 236--243, Core/Basic.lean}
...
\textbf{Lines 245--247, Core/Basic.lean}
...
\begin{itemize}
  ...
  \item \texttt{OpStrictMonoLeft op} (line 146): $\forall y,\; \text{StrictMono}(\lambda x.\, \text{op}(x, y))$
  \item \texttt{OpStrictMonoRight op} (line 150): $\forall x,\; \text{StrictMono}(\lambda y.\, \text{op}(x, y))$
  \item \texttt{OpIdentLeft op e} (line 154): $\forall x,\; \text{op}(e, x) = x$
  \item \texttt{OpIdentRight op e} (line 158): $\forall x,\; \text{op}(x, e) = x$
  \item \texttt{IdentIsMin e} (line 162): $\forall x,\; e \leq x$
\end{itemize}
...
\textbf{File}: \texttt{KnuthSkilling/Core/Algebra.lean}
...
\textbf{Lines 374--375, Core/Basic.lean}
...
\textbf{File}: \path{KnuthSkilling/Additive/Axioms/AnomalousPairs.lean}
...
\begin{lstlisting}[caption={Separation axiom (identity-free, Algebra.lean:335)},numbers=none]
...
\textbf{File}: \texttt{KnuthSkilling/Additive/Axioms/AnomalousPairs.lean}
...
\item \texttt{Additive/Proofs/GridInduction/Core/} (grid infrastructure)
  \begin{itemize}
  \item \texttt{Additive/Proofs/GridInduction/Core/MultiGrid.lean}: \texttt{AtomFamily}, \texttt{MultiGridRep}, grid representations
  \item \texttt{Additive/Proofs/GridInduction/Core/Induction/}: Inductive extension theorems (\texttt{Construction}, \texttt{ThetaPrime}, \texttt{DeltaShift}, \texttt{Goertzel})
  \item \texttt{Additive/Proofs/GridInduction/Core/OneDimensional.lean}: Base case (single atom)
  \item \texttt{Additive/Proofs/GridInduction/Core/Prelude.lean}: Foundational lemmas
  \end{itemize}
\end{itemize}

The grid path entry point is \texttt{Additive/Proofs/GridInduction/Globalization.lean}, which imports \texttt{Additive/Proofs/GridInduction/Core/All.lean}.
...
\textbf{Lines 27--33, GlobalizationInterface.lean}
...
class RepresentationGlobalization (alpha : Type*)
    [KnuthSkillingMonoidBase alpha] : Prop where
  exists_Theta :
    exists Theta : alpha -> Real,
      (forall a b : alpha, a <= b <-> Theta a <= Theta b) /\
      Theta ident = 0 /\
...
The instance \texttt{representationGlobalization\_of\_KSSeparationStrict} (lines 101--850, Globalization.lean)
...
\texttt{DeltaSpec\_unique} (line 755, \texttt{Additive/Proofs/GridInduction/Core/Induction/Construction.lean}).
...
\texttt{representationGlobalization\_of\_KSSeparationStrict}
\\
(starts at line 101, \path{Additive/Proofs/GridInduction/Globalization.lean})
...
\item \textbf{Identity-free}: Uses \texttt{Theta\_cuts\_pnat} with $\mathbb{N}^+$ iteration
  \begin{itemize}
  \item \texttt{Theta\_cuts\_pnat} (line 1436): Definition via Dedekind cuts using $\mathbb{N}^+$ iteration
  \item \texttt{Theta\_cuts\_pnat\_strictMono} (line 1532): Strict monotonicity (fully proven)
  \item \texttt{Theta\_cuts\_pnat\_add} (line 1638): Additivity (fully proven)
...
\textbf{Lines 85--112, Additive/Proofs/DirectCuts/Main.lean}
...
theorem associativity_representation_cuts
    (alpha : Type*) [KnuthSkillingAlgebraBase alpha] [KSSeparation alpha]
...
\begin{itemize}
\item \textbf{Grid proof}: 6,604 lines (ThetaPrime: 3,609 + MultiGrid: 2,995 --- induction machinery, extension lemmas, path independence)
\item \textbf{Cuts proof}: 2,079 lines (DirectCuts.lean --- direct construction, no induction)
\end{itemize}
...
\textbf{File}: \texttt{KnuthSkilling/Information/Divergence.lean}
...
\textbf{File}: \path{KnuthSkilling/Probability/ConditionalProbability/Basic.lean}
...
\textbf{Lines 127--142 (ChainingOp structure), Basic.lean}
...
\textbf{File}: \texttt{KnuthSkilling/Core/ScaleCompleteness.lean}
...
\texttt{KnuthSkilling/Bridges/MathlibProbability.lean}.
\end{remark}
...
\textbf{File}: \texttt{KnuthSkilling/Information/InformationEntropy.lean}
...
\textbf{Unified view (project-wide, optional)}: The repo also formalizes Shannon's 1948 uniqueness theorem, Faddeev's 1956 uniqueness theorem, the Shannon--Khinchin axiom interface and Faddeev bridge, plus bridges to mathlib's measure-theoretic KL divergence. The entrypoint \path{InformationTheory/EntropyKL.lean} records the equivalences between these approaches (see Appendix: Supplementary Derivation Paths).
...
\textbf{Lines 19--22, ProbabilityTheory/Foundations/Distributions/ProbDist.lean}
...
{\footnotesize\path{ProbabilityTheory/Foundations/Distributions/ProbDist.lean}} and re-exported as a KS-facing alias
...
\textbf{Directory}: \texttt{KnuthSkilling/Counterexamples/}
...
All paths are relative to \texttt{KnuthSkilling/}.
...
\begin{itemize}
\item \texttt{selection\_theorem} (\texttt{Algebra/TwoDimClassification.lean:427}) --- $\mu < 0$ gives QM Born rule
\item \texttt{mean\_bornRule\_sum\_unit\_phases} (\texttt{Core/SymmetricalFoundation.lean:303}) --- Born rule from averaging
\item \texttt{classification\_trichotomy} (\texttt{Algebra/TwoDimClassification.lean:293}) --- Exactly 3 algebra classes
\end{itemize}
...
\textbf{Curated import surface}: \path{InformationTheory/EntropyKL.lean}
...
\textbf{File}: \path{InformationTheory/ShannonEntropy/Interface.lean}
...
\textbf{File}: \path{InformationTheory/ShannonEntropy/MeasureTheoreticBridge.lean}
...
\textbf{Entry point}: \path{InformationTheory/ShannonEntropy/Main.lean}
...
\textbf{Lines 6481--6486, ShannonEntropy/Faddeev.lean}
...
\textbf{File}: \path{KnuthSkilling/ShoreJohnson/SystemIndependence.lean}
...
\textbf{File}: \path{KnuthSkilling/ShoreJohnson/KL.lean}
...
\textbf{File}: \path{KnuthSkilling/ShoreJohnson/Bridge.lean}
...
\textbf{File}: \path{KnuthSkilling/Bridges/ShoreJohnsonVariationalBridge.lean}
...
\textbf{Entry point}: \path{ProbabilityTheory/Cox.lean}

\medskip
\textbf{File}: \path{ProbabilityTheory/Cox/Basic.lean}
...
\textbf{File}: \path{ProbabilityTheory/Cox/ProductRuleDerivation.lean}

\begin{theorem}[\texttt{cox\_productRule}]
\textbf{Lines 2348--2350, Cox/ProductRuleDerivation.lean}
...
\textbf{Lines 2390--2401, Cox/ProductRuleDerivation.lean}
...
\textbf{Lines 3373--3375, Cox/ProductRuleDerivation.lean}
...
\textbf{File}: \path{ProbabilityTheory/Cox/ProbabilityCalculusBridge.lean}
...
\textbf{File}: \path{ProbabilityTheory/Cox/ProbabilityCalculusIntegration.lean}
...
instance (M : CoxProbabilityModel alpha) :
    KnuthSkilling.Probability.ProbabilityCalculus.ProbabilityCalculusClass alpha
      M.valuation where
...
\textbf{File}: \path{KnuthSkilling/Examples/CoinDie.lean}
...
\textbf{File}: \path{KnuthSkilling/Examples/PreciseVsImprecise.lean}
...
export LAKE_JOBS=3
nice -n 19 lake build +KnuthSkilling
\end{lstlisting}

For the FOI reviewer entrypoint only:
\begin{lstlisting}[language=bash]
export LAKE_JOBS=3
nice -n 19 lake build +KnuthSkilling.FoundationsOfInference
\end{lstlisting}
...
ulimit -v 6291456
export LAKE_JOBS=1
nice -n 19 lake build +KnuthSkilling.Additive.Proofs.GridInduction.Main
\end{lstlisting}
