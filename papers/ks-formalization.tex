...
A key finding of this formalization is that the \textbf{NoAnomalousPairs} condition from classical ordered semigroup theory (Alimov 1950, Fuchs 1963) is sufficient for the representation theorem, and is \emph{weaker} than K\&S's separation property.
...
\textbf{Files}: \texttt{RepresentationTheorem/Core/Induction/ThetaPrime.lean} ($\sim$2500 lines), \\
\hspace*{2em} \texttt{RepresentationTheorem/Core/MultiGrid.lean} ($\sim$800 lines)
...
\textbf{Files}: \texttt{RepresentationTheorem/Alternative/DirectCuts.lean} ($\sim$900 lines)
...
\textbf{Files}: \texttt{Separation/HolderEmbedding.lean} ($\sim$320 lines) + Eric's library ($\sim$1500 lines)
...
\textbf{File}: \texttt{RepresentationTheorem/Comparison.lean}
...
\item \textbf{Product-equation route} (closest to K\&S's presentation):
  ...
  (Lean: \texttt{ProductTheorem/Main.lean}.)
\item \textbf{Lean-friendly "avoid Appendix A again" route}:
  ...
  (Lean: \texttt{ProductTheorem/AczelTheorem.lean}.)
...
\textbf{Axiom audit:} We checked all \texttt{ProductTheorem/*.lean} files for \texttt{axiom}, \texttt{sorry}, or \texttt{admit}---none found.
...
\item \textbf{K\&S's Fibonacci proof}: COMPLETE.  The full Fibonacci recurrence argument is formalized
  in \texttt{FibonacciProof.lean}, including:
  \begin{enumerate}
...
\item \textbf{Archived alternatives}: Two incomplete approaches (deriving StrictMono from Continuous,
  and deriving StrictMono from weak Monotone) are preserved in \texttt{FibonacciProofScratch.lean}
...
(\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/VariationalTheorem.lean}), which models
...
\ (\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/VariationalTheorem.lean}).
...
\texttt{KSVariationalGenerality} (\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/VariationalTheorem.lean}),
...
\end{itemize}
These results are proved in \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnsonKL.lean}.
For convenience, the project re-exports the key Shore--Johnson lemmas we use in
\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnsonTheorem.lean}.
...
\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnsonBridge.lean}.
...
This is formalized in \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/Divergence.lean} with proofs of non-negativity, zero iff equal, and asymmetry.
...
\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/ConditionalProbability/Basic.lean}.
...
\texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/InformationEntropy.lean}.
...
\item \textbf{Mathlib KL bridge (finite $\to$ general)}: we show our \texttt{klDivergenceTop} on \texttt{Fin n}
  ...
  \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/InformationEntropyMathlib.lean}).
...
\item \textbf{Mathlib KL bridge (countable discrete)}: for sequences on $\mathbb{N}$ we connect the K\&S
  ...
  \texttt{Mettapedia/ProbabilityTheory/KnuthSkilling/DivergenceMathlib.lean}).
...
\item \textbf{Discovery of minimal assumptions}: The \texttt{NoAnomalousPairs} condition from classical ordered semigroup theory (Alimov 1950) is \emph{sufficient} for the representation theorem and is \emph{weaker} than K\&S's separation property.
...
\item Shortest local codebase ($\sim$420 lines vs.\ $\sim$4000 for grid approach)
...
Thus, \texttt{NoAnomalousPairs} alone suffices for the representation theorem---no \texttt{KSSeparation} needed!

\textbf{Files}:
\begin{itemize}
\item \texttt{Separation/HolderEmbedding.lean}: Bridge to Eric's library
\item \texttt{Separation/AnomalousPairs.lean}: NoAnomalousPairs definition
\item \texttt{RepresentationTheorem/Comparison.lean}: Unified interface
\end{itemize}
...
\texttt{ProductTheorem/AczelTheorem.lean}.
