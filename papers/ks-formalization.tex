...
\begin{abstract}
We present a complete formalization in Lean 4.28.0 (Mathlib v4.28.0) of Knuth \& Skilling's axiomatic foundations for inference (2012).
...
\begin{lstlisting}[caption={Core K\&S algebra axioms (identity-free hierarchy)}]
...
Let $\alpha$ be a type with \texttt{KSSemigroupBase}.
...
\end{enumerate}
When an identity element is present (\texttt{KnuthSkillingMonoidBase} or \texttt{KnuthSkillingAlgebraBase}), we additionally get $\Theta(\text{ident}) = 0$.
\end{theorem}
...
\textbf{Files}: \texttt{Additive/Proofs/GridInduction/Core/Induction/ThetaPrime.lean} (3,609 lines), \\
\hspace*{2em} \texttt{Additive/Proofs/GridInduction/Core/MultiGrid.lean} (2,995 lines)
...
\begin{enumerate}
\item Show \texttt{KnuthSkillingMonoidBase} provides an \texttt{IsOrderedCancelSemigroup} instance
...
instance holder_hasRepresentationTheorem [KSSemigroupBase α] [NoAnomalousPairs α]
instance grid_hasRepresentationTheorem [KSSemigroupBase α] [RepresentationGlobalizationSemigroup α]
instance cuts_hasRepresentationTheorem [KnuthSkillingAlgebraBase α] [KSSeparation α] [KSSeparationStrict α]
\end{lstlisting}
...
(\texttt{KnuthSkilling/Variational/Main.lean}), which models
...
\ (\texttt{KnuthSkilling/Variational/Main.lean}).
...
\texttt{KSVariationalGenerality} (\texttt{KnuthSkilling/Variational/Main.lean}),
...
These counterexamples are formalized in \texttt{KnuthSkilling/Counterexamples/VariationalNonsmooth.lean}.
...
The existence of non-linear additive solutions is shown by a standard Hamel-basis construction; we formalize this as a counterexample generator in \texttt{KnuthSkilling/Counterexamples/CauchyPathology.lean}.
...
in \texttt{KnuthSkilling/ShoreJohnson/SystemIndependence.lean} (re-exported via \texttt{ShoreJohnson/Theorem.lean}).
...
\end{itemize}
These results are proved in \texttt{KnuthSkilling/ShoreJohnson/KL.lean}.
For convenience, the project re-exports the key Shore--Johnson lemmas we use in
\texttt{KnuthSkilling/ShoreJohnson/Theorem.lean}.
...
\texttt{KnuthSkilling/ShoreJohnson/Bridge.lean}.
...
This is formalized in \texttt{KnuthSkilling/Information/Divergence.lean} with proofs of non-negativity, zero iff equal, and asymmetry.
...
\texttt{KnuthSkilling/Probability/ConditionalProbability/Basic.lean}.
...
\texttt{KnuthSkilling/Information/InformationEntropy.lean}.
...
\begin{itemize}
\item Shannon's original 1948 axioms + uniqueness (Appendix~2 of Shannon): \texttt{InformationTheory/ShannonEntropy/Shannon1948.lean} (sorry-free).
\item Faddeev's 1956 axiom system + uniqueness theorem: \texttt{InformationTheory/ShannonEntropy/Faddeev.lean} (sorry-free).
\item Shannon--Khinchin's 1957 axiom interface and the bridge to/from Faddeev:
\texttt{InformationTheory/ShannonEntropy/ShannonKhinchin.lean} and
\texttt{InformationTheory/ShannonEntropy/Equivalence.lean} (sorry-free; the direct S--K uniqueness theorem is not duplicated in that file).
\end{itemize}
...
\item \textbf{Mathlib KL bridge (finite $\to$ general)}: we show our \texttt{klDivergenceTop} on \texttt{Fin n}
  ...
  \texttt{KnuthSkilling/Information/InformationEntropyMathlib.lean}).
...
\item \textbf{Mathlib KL bridge (countable discrete)}: for sequences on $\mathbb{N}$ we connect the K\&S
  ...
  \texttt{KnuthSkilling/Information/DivergenceMathlib.lean}).
...
theorem op_archimedean_of_separation
    [KSSeparation α] (a : α) (x : α) (ha : ident < a) :
...
We compare three foundational approaches to probability, all formalized in this repository.
...
\textbf{Formalization}: See \texttt{ProbabilityTheory/Cox/Basic.lean}
...
\textbf{Our contribution}: We show that \texttt{KnuthSkillingMonoidBase} satisfies all the hypotheses of Eric's theorem:
\begin{itemize}
...
\textbf{Formalization}: See \texttt{ProbabilityTheory/Hypercube/Basic.lean}
...
\textbf{Formalization}: See \texttt{ProbabilityTheory/Hypercube/NovelTheories.lean} and \\
\hspace*{2em} \texttt{ProbabilityTheory/Hypercube/NeighborTheories.lean}
...
\texttt{KnuthSkilling/Multiplicative/FunctionalEquation.lean},
...
See \texttt{KnuthSkilling/Examples/} (and the nearby \texttt{Bridges/} and \texttt{Exploration/} folders) for:
\begin{itemize}
