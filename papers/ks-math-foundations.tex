...
in the \texttt{KnuthSkilling} namespace.
...
\hline
H\"older embedding & \texttt{holder\_embedding\_of\_noAnomalousPairs} & \path{Additive/Proofs/OrderedSemigroupEmbedding/HolderEmbedding.lean} \\
\hline
$\oplus$, $\otimes$ & \texttt{op}, \texttt{tensor} & \path{Core/Basic.lean}, \path{Multiplicative/Basic.lean} \\
\hline
$x^{(n)}$ (iteration) & \texttt{iterate\_op\_pnat} & \path{Core/Basic.lean} \\
\hline
Additive representation & \texttt{HasRepresentationTheorem} & \path{Additive/Representation.lean} \\
\hline
Product representation & \texttt{tensor\_coe\_eq\_mul\_div\_const} & \path{Multiplicative/Main.lean} \\
\hline
Variational theorem & \texttt{variationalEquation\_solution\_measurable} & \path{Variational/Main.lean} \\
\hline
KL divergence bridge & \texttt{klDiv\_toMeasure\_eq\_divergenceInfTop} & \path{Information/DivergenceMathlib.lean} \\
\hline
...
\hline
$\oplus \to +$ (derived) & \texttt{op\_is\_addition\_via\_Θ} & \path{Additive/Axioms/OpIsAddition.lean} \\
\hline
