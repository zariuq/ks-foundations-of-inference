...
This file contains a collection of *formalized constraints* obtainable from `KSSeparation` when
assuming noncommutativity, but the final contradiction step is currently missing.

See `.../Core/CommutativityConjecture.lean` for the up-to-date status and the open conjecture. -/

/-- Conjecture: `KSSeparation` implies commutativity. -/
def SeparationImpliesCommutativeConjecture : Prop :=
  ∀ x y : α, op x y = op y x
