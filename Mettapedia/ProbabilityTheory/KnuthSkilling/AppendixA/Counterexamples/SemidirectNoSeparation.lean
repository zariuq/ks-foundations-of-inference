...
This file intentionally lives under `Mettapedia/ProbabilityTheory/KnuthSkilling/AppendixA/Counterexamples/`.
...
theorem op_not_comm : KnuthSkillingAlgebra.op exX exY ≠ KnuthSkillingAlgebra.op exY exX := by
  -- exX ⊕ exY = (3, 1 + 2^1*1) = (3, 3)
  -- exY ⊕ exX = (3, 1 + 2^2*1) = (3, 5)
  simp [SD.exX, SD.exY, SD.op, SD.baseOp, KnuthSkillingAlgebra.op]
  decide
...
theorem fst_iterate_op_coe (p : SDBase) :
  ...
  induction n with
  | zero =>
    simp [KnuthSkillingAlgebra.iterate_op, KnuthSkillingAlgebra.ident, SD.ident, SD.fst]
  | succ n ih =>
    -- iterate_op (n+1) = op p (iterate_op n)
    -- fst(op p z) = u + fst(z)
    have : SD.fst (KnuthSkillingAlgebra.op (p : SD) (iterate_op (p : SD) n)) =
        (p.1 : ℕ) + SD.fst (iterate_op (p : SD) n) := by
      -- reduce to the concrete `SD.fst_op_left_some`
      simpa [KnuthSkillingAlgebra.op, SD.op, SD.fst] using (SD.fst_op_left_some p (iterate_op (p : SD) n))
