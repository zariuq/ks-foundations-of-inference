...
structure IntervalMeasure (α : Type*) [KnuthSkillingAlgebra α] where
  ...
  sub_additive : ∀ x y, (μ x).lower + (μ y).lower ≤ (μ (KnuthSkillingAlgebra.op x y)).lower
  super_additive : ∀ x y, (μ (KnuthSkillingAlgebra.op x y)).upper ≤ (μ x).upper + (μ y).upper
...
theorem IntervalMeasure.containment {α : Type*} [KnuthSkillingAlgebra α]
    ...
    (M.μ (KnuthSkillingAlgebra.op x y)).lower ≥ (M.μ x).lower + (M.μ y).lower ∧
    (M.μ (KnuthSkillingAlgebra.op x y)).upper ≤ (M.μ x).upper + (M.μ y).upper :=
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.AppendixA.Counterexamples.IntervalApproach
