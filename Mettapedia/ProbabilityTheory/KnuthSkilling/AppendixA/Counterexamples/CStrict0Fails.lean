...
So the strict C-side inaccessibility property (our `AppendixA34Extra.C_strict0`) fails.
-/
...
theorem C_strict0_fails_semidirect :
  ...
  · -- Show `d < a^2`.
    have hd_lt_a2 : iterate_op SD.exY 1 < iterate_op SD.baseA 2 := by
      ...
      simpa [KnuthSkillingAlgebra.iterate_op_one, KnuthSkillingAlgebra.iterate_op, KnuthSkillingAlgebra.op,
        SD.op, SD.baseOp, SD.exY, SD.baseA] using
        (show (SD.exY : SD) < KnuthSkillingAlgebra.op SD.baseA SD.baseA by
          simp [KnuthSkillingAlgebra.op, SD.op, SD.baseOp, SD.exY, SD.baseA]
          decide)
        ...
        have : iterate_op SD.exY 1 < iterate_op SD.baseA 2 := by
          ...
          simpa [KnuthSkillingAlgebra.iterate_op_one] using
            (show (SD.exY : SD) < iterate_op SD.baseA 2 by
              -- compute `a^2 = a ⊕ a`
              simpa [KnuthSkillingAlgebra.iterate_op, KnuthSkillingAlgebra.op, SD.op, SD.baseOp, SD.exY, SD.baseA]
                using (show (SD.exY : SD) < KnuthSkillingAlgebra.op SD.baseA SD.baseA by
                  simp [KnuthSkillingAlgebra.op, SD.op, SD.baseOp, SD.exY, SD.baseA]
                  decide))
