...
namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ConditionalProbability

open Classical
open Mettapedia.ProbabilityTheory.KnuthSkilling.ProductTheorem
...
then invoking `ProductTheorem.tensor_coe_eq_mul_div_const_of_tensorRegularity`. -/
...
theorem chaining_gives_productEquation
    ...
    ∃ (Ψ' : ℝ → ℝ) (ζ : ℝ → ℝ → ℝ),
      (∀ x, Ψ' (Θ x) = x) ∧
      ProductTheorem.ProductEquation Ψ' ζ := by
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.ConditionalProbability
