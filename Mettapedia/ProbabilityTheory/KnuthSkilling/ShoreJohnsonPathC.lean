...
structure SJPathCAssumptions where
  ...
  /-- An objective-level system-independence normalization for the atom divergence `d` (stronger
  than the operator-level SJ4 statement, and not yet derived from it). -/
  sjAtom : ShoreJohnson.SJSystemIndependenceAtom d
...
theorem pathC_log_on_probabilities (h : SJPathCAssumptions) :
  ...
  exact ShoreJohnsonTheorem.d_one_eq_const_mul_log_of_SJ4 h.d h.sjAtom h.d1Meas
...
theorem pathC_infer_iff_isMinimizer_ofAtom_const_mul_klAtom_of_posPrior (h : SJPathCAssumptions) :
  ...
  have hKl :
    ...
    isMinimizer_ofAtom_iff_ofAtom_const_mul_klAtom_of_posPrior
      h.d C hC h.sjAtom.regularAtom p hp (ShoreJohnsonConstraints.toConstraintSet cs) q
  constructor
  · intro hInfer
