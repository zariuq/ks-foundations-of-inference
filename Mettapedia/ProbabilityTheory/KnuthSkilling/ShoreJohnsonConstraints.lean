...
namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonConstraints
...
theorem twoBlockConstraints_complete_set
  ...
  | inl i₁ =>
      ...
      have : w * (P₁.p i₁) = Q.p (inlFin (n := n) (m := m) i₁) := by
        -- `P₁.p i₁ = Q(left)/w`
        simp [P₁, mul_div_cancel₀, hwne]
      simp [ShoreJohnsonInference.ProbDist.twoBlock, inlFin, this]
  | inr i₂ =>
      ...
      have : (1 - w) * (P₂.p i₂) = Q.p (inrFin (n := n) (m := m) i₂) := by
        simp [P₂, mul_div_cancel₀, h1w_ne]
      simp [ShoreJohnsonInference.ProbDist.twoBlock, inrFin, this]
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonConstraints
