...
namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonAppendixKL
...
open Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonBridge
...
lemma klAtom_eq_mul_gKL (w u : ℝ) (hw : 0 < w) (hu : 0 < u) :
...
theorem stationaryEV_of_isMinimizer_ofAtom_klAtom {n : ℕ}
        ...
        have hsat : satisfiesSet cs qT := by
          ...
          calc
            ...
            _ = (cs.get k).rhs + t * 0 := by
              -- Rewrite the kernel condition under the affine map `s ↦ rhs + t*s`.
  ...
  have hK :
    ...
    simpa [K, gradAt_apply_single] using (gradAt_apply_single (p := p) (q := q) i)
  ...
  have hL :
    ...
    have hnone : L none (Pi.single i (1 : ℝ) : Fin n → ℝ) = 1 := by
      simpa [L, constraintForm_apply_single_none] using constraintForm_apply_single_none (cs := cs) i
    have hsome : ∀ k : Fin cs.length, L (some k) (Pi.single i (1 : ℝ) : Fin n → ℝ) = (cs.get k).coeff i := by
      intro k
      simpa [L, constraintForm_apply_single_some] using constraintForm_apply_single_some (cs := cs) k i
    -- Substitute.
...
/-- **Key KL stationarity theorem (PROVEN, not assumed)**.
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonAppendixKL
