...
namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnson.Appendix
...
def satisfiesFn {n : ℕ} (c : Constraints.EVConstraint n) (q : Fin n → ℝ) : Prop :=
  (∑ i : Fin n, q i * c.coeff i) = c.rhs

theorem satisfiesFn_shift2_of_coeff_eq {n : ℕ} (c : Constraints.EVConstraint n)
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnson.Appendix
