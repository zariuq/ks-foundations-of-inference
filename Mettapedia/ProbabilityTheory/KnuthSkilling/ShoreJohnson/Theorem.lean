...
1. **System-independence ⇒ logarithm on probabilities.**
   In `.../ShoreJohnson/SystemIndependence.lean` we formalize a core lemma ("Dirac extraction") showing that an
...
2. **KL uniqueness (up to scale) inside a ratio-form class.**
   In `.../KL.lean` we formalize a Cauchy/log rigidity theorem on `(0,∞)` and apply it
...
`Mettapedia/ProbabilityTheory/KnuthSkilling/ShoreJohnson/Bridge.lean`.
-/

namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnson.Theorem
...
theorem d_one_eq_const_mul_log_of_SJ4
  ...
  SystemIndependence.d_one_eq_const_mul_log_of_measurable d hSJ hMeas
...
theorem exists_ratioForm_mulCauchyOnPos_not_const_mul_klAtom :
  ...
  KL.exists_ratioForm_mulCauchyOnPos_not_const_mul_klAtom

end Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnson.Theorem
