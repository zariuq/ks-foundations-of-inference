...
theorem shift2_deriv_ofAtom
    ...
    (hd : ∀ q p : ℝ, HasDerivAt (fun t => d t p) (d' q p) q)
    {n : ℕ} (p q : ProbDist n) (i j : Fin n) (hij : i ≠ j)
    (_hp : ∀ k, 0 < p.p k) (hq : ∀ k, 0 < q.p k) :
    HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q i j hij t) p)
  ...
  have hEventuallyEq : (fun t => ∑ k : Fin n, d (shift2 q.p i j t k) (p.p k)) =ᶠ[𝓝 (q.p i)]
      (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q i j hij t) p) := by
    ...
    intro t ht
    simp only [ShoreJohnsonObjective.ofAtom]
    congr 1
    funext k
...
theorem deriv_local_ofAtom
    (d : ℝ → ℝ → ℝ)
    (d' : ℝ → ℝ → ℝ)
    (_hd : ∀ q p : ℝ, HasDerivAt (fun t => d t p) (d' q p) q)
    {n m : ℕ} (p : ProbDist n) (q : ProbDist n) (p' : ProbDist m) (q' : ProbDist m)
...
noncomputable def ExtractGRegularity.ofAtom
  ...
  deriv_local := fun {n m} p q p' q' i j i' j' hij hi'j' hp hq hp' hq' h_qi h_pi h_qj h_pj => by
    ...
    let ex1 : ∃ L, HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q i j hij t) p) L (q.p i) :=
      ⟨d' (q.p i) (p.p i) - d' (q.p j) (p.p j), hL⟩
    let ex2 : ∃ L, HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q' i' j' hi'j' t) p') L (q'.p i') :=
      ⟨d' (q'.p i') (p'.p i') - d' (q'.p j') (p'.p j'), hL'⟩
  ...
  cocycle := fun {n} p q i j k hij hjk hik hp hq => by
    ...
    let ex_ij : ∃ L, HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q i j hij t) p) L (q.p i) :=
      ⟨d' (q.p i) (p.p i) - d' (q.p j) (p.p j), hLij⟩
    let ex_jk : ∃ L, HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q j k hjk t) p) L (q.p j) :=
      ⟨d' (q.p j) (p.p j) - d' (q.p k) (p.p k), hLjk⟩
    let ex_ik : ∃ L, HasDerivAt (fun t => (ShoreJohnsonObjective.ofAtom d).D (shift2ProbClamp q i k hik t) p) L (q.p i) :=
...
1. **Projection/Embedding lemmas**: Show how to "lift" a 2-element problem to n dimensions
2. **shift2 commutation**: Show shift2 on the embedded system equals embedded shift2
3. **Derivative transport**: Show derivatives are preserved under embedding
4. **SJ3 application**: Use `eq_twoBlock_of_isMinimizer_twoBlock` to show minimizers match
5. **Chain rule for shift2**: Show cocycle holds via composition of shift2 curves

These are technical lemmas about the twoBlock structure and shift2 calculus. Once proven,
`extractGRegularity_from_sj_axioms` provides `ExtractGRegularity F` from SJ axioms + diff.

**Current status**: The locality step is now an explicit assumption (`DerivLocalAssumption`).
Deriving it from SJ3 remains the main open technical task (twoBlock + projection + derivative
transport). The cocycle is handled via the explicit `hPartial` assumption.

**Key insight**: The gap is explicit and mathematically well-defined. No hidden assumptions.
-/
...
SJ Axioms (SJ1, SJ2, SJ3) + Differentiability + Gradient Structure
    ⇓ (extractGRegularity_from_sj_axioms + DerivLocalAssumption + EVRichness)
ExtractGRegularity F  (locality + cocycle + has_shift2_deriv)
    ⇓ (exists_gradient_representation - PROVEN, 0 sorries)
...
Sum-Form Objective (ofAtom d)
    ⇓ (ExtractGRegularity.ofAtom - PROVEN, 0 sorries)
...
theorem sumForm_complete_regularity
    ...
    (d' : ℝ → ℝ → ℝ)
    (hd : ∀ q p : ℝ, HasDerivAt (fun t => d t p) (d' q p) q) :
    ExtractGRegularity (ShoreJohnsonObjective.ofAtom d) :=
...
1. 🚧 `DerivLocalAssumption`: derive minimizer-scoped locality (A1–A3) from SJ3 via twoBlock/projection calculus
...
**TODO**:
- twoBlock embedding infrastructure (for deriving `DerivLocalAssumption` from SJ3)
- Derivative-transport lemmas under projection/permutation

**Mathematical insight**: The formalization reveals that:
- The "sum-form → regularity" direction is TRIVIAL (pure algebra, 0 sorries)
- The "SJ axioms → sum-form" direction requires REAL WORK (twoBlock + permutation calculus)
