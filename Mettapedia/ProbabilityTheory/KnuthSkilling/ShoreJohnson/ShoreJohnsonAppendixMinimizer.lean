import Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnson.ShoreJohnsonAppendix
...
This is a building block for the (still WIP) Shore–Johnson Theorem I proof.
-/

namespace Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonAppendixMinimizer
...
theorem shift2ProbClamp_swap {n : ℕ} (q : ProbDist n) (i j : Fin n) (hij : i ≠ j)
  ...
  -- Now use the shift2_swap identity from ShoreJohnsonAppendix
...
theorem shift2Clamp_permute {n : ℕ} (σ : Equiv.Perm (Fin n)) (q : ProbDist n) (i j : Fin n)
    (t : ℝ) :
    shift2Clamp (ShoreJohnsonInference.ProbDist.permute σ q) (σ i) (σ j) t = shift2Clamp q i j t := by
  -- Both clamps depend only on `t` and the sum `q_i + q_j`, which is permutation-invariant.
  dsimp [shift2Clamp, ShoreJohnsonInference.ProbDist.permute]
  simp [add_comm, add_left_comm, add_assoc]

theorem permute_shift2ProbClamp {n : ℕ} (σ : Equiv.Perm (Fin n)) (q : ProbDist n)
    (i j : Fin n) (hij : i ≠ j) (t : ℝ) :
    ShoreJohnsonInference.ProbDist.permute σ (shift2ProbClamp q i j hij t) =
      shift2ProbClamp (ShoreJohnsonInference.ProbDist.permute σ q) (σ i) (σ j)
        (σ.injective.ne_iff.mpr hij) t := by
  classical
  apply ProbDist.ext
  intro k
  -- Unfold both sides to `shift2` on coordinate functions.
  simp only [ShoreJohnsonInference.ProbDist.permute_apply, shift2ProbClamp_apply]
  -- The clamped parameter is invariant under permutation.
  have hclamp :
      shift2Clamp (ShoreJohnsonInference.ProbDist.permute σ q) (σ i) (σ j) t =
        shift2Clamp q i j t :=
    shift2Clamp_permute (σ := σ) (q := q) (i := i) (j := j) (t := t)
  -- Now split cases on whether `k` hits the shifted coordinates.
  by_cases hk_si : σ⁻¹ k = i
  · -- Then k = σ i.
    have hk : k = σ i := by
      -- Apply σ to both sides of σ⁻¹ k = i.
      simpa using congrArg σ hk_si
    subst hk
    simp [shift2, hclamp]
  · by_cases hk_sj : σ⁻¹ k = j
    · have hk : k = σ j := by
        simpa using congrArg σ hk_sj
      subst hk
      simp [shift2, hk_si, hclamp, hij]
    · -- Otherwise, it's an "other" coordinate.
      have hk_i : k ≠ σ i := by
        intro hk
        apply hk_si
        simpa [hk] using congrArg σ⁻¹ hk
      have hk_j : k ≠ σ j := by
        intro hk
        apply hk_sj
        simpa [hk] using congrArg σ⁻¹ hk
      simp [shift2, hk_si, hk_sj, hk_i, hk_j, hclamp]
...
end Mettapedia.ProbabilityTheory.KnuthSkilling.ShoreJohnsonAppendixMinimizer
