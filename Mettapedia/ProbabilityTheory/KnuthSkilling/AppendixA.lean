...
lemma separation_property {k : ℕ} {F : AtomFamily α k} (R : MultiGridRep F)
  ...
  -- Step 4: From iterate inequality, derive Θ inequality
  -- Note: We don't claim μ(F, mA · rA) = (μ(F,rA))^mA (that's false!)
  -- Instead, we show the ORDERING transfers through the grid

  -- The key: if (μ(F,rA))^mA < (μ(F,rC))^mC, and these are comparable
  -- via the grid structure, then their Θ-values respect the order

  -- For now, we assume iterate_op preserves order through grid comparisons
  -- This is a deep property that requires the full grid construction
  sorry -- TODO: Complete the bridge from iterate order to Θ order via grid
...
lemma A_stat_le_delta_of_B_empty {k : ℕ} {F : AtomFamily α k} (R : MultiGridRep F)
  ...
  have h_bdd : BddAbove AStats := by
    ...
    exact le_of_lt (separation_property R hd hu' hrA' huC hrC)
...
lemma delta_le_C_stat_of_B_empty {k : ℕ} {F : AtomFamily α k} (R : MultiGridRep F)
  ...
  exact le_of_lt (separation_property R hd hu' hrA' hu hrC)
  ...
  **UNIFORM PROOF STRATEGY** (GPT-5.1 Pro / K&S Appendix A):
  ...
  · -- Upper bound: θr - θs ≤ Δ * δ
    ...
    · -- s_old > ident case: Use B-witness structure and R.add
      ...
      /-
      **Key insight**: mu_add : mu F (r+s) = op (mu F r) (mu F s) is FALSE without commutativity!
      The mu function computes: ident ⊕ a₀^{r₀} ⊕ a₁^{r₁} ⊕ ...
      For mu F (r+s): terms are interleaved (a₀^{r₀} ⊕ a₀^{s₀}) ⊕ (a₁^{r₁} ⊕ a₁^{s₁}) ⊕ ...
      For op (mu F r) (mu F s): all r-terms then all s-terms
      These differ without commutativity.

      This is the CIRCULARITY in K&S: we need commutativity for mu_add, but commutativity
      comes FROM the representation theorem. The resolution is:
      - Base case (k=1): Single atom, trivially commutative
      - Inductive step (k→k+1): Use accuracy lemma to pin δ, then derive commutativity

      For this lemma, the correct approach is the ACCURACY LEMMA argument:
      As the A/C gap → 0 (via accuracy), the bounds squeeze to equality.
      This is the hardest part of K&S Appendix A (Sections A.3.4-A.3.5).
      -/
      sorry -- TODO: Complete via accuracy lemma (K&S Appendix A, Sections A.3.4-A.3.5)
  ...
  · -- Lower bound: Δ * δ ≤ θr - θs
    ...
    · -- s_old > ident: Same structure as upper bound case
      ...
      -- We have: Δ·δ ≤ θr (from C-bound on r) but need Δ·δ ≤ θr - θs
      -- This requires θs ≤ θr - Δ·δ, which needs an upper bound on θs

      -- Same situation as upper bound: mu_add is FALSE without commutativity.
      -- The correct approach is the accuracy lemma (K&S Appendix A, A.3.4-A.3.5).
...
lemma Theta'_well_defined
  ...
  · -- Case t < u: The "trade argument"
    ...
    have h_trade_eq := delta_shift_equiv R hd hδA hδC hδB hΔ_pos hμ_trade
...
theorem extend_grid_rep_with_atom
  ...
  · -- Layer B: Construct R' (representation on extended grid)
    ...
    have Theta'_increment_t : ∀ (r_old : Multi k) (t : ℕ),
      ...
      -- The chosen witnesses are exactly joinMulti r_old (t+1) and joinMulti r_old t
      -- because mu F' (joinMulti r_old n) is in the grid with witness joinMulti r_old n
      sorry -- TODO: Use Classical.choose_spec to show witnesses = joinMulti, then ring
    ...
    have Theta'_strictMono_same_t : ∀ (r_old_x r_old_y : Multi k) (t : ℕ),
      ...
      sorry -- TODO: Use Classical.choose_spec + R.strictMono + linarith
    ...
    have h_strictMono : StrictMono Θ' := by
      intro ⟨x, hx⟩ ⟨y, hy⟩ hxy
      simp only [Θ']
      -- Extract witnesses and split into old + new components
      ...
      -- The ordering x < y induces constraints on (r_old_x, t_x) vs (r_old_y, t_y)
      -- via mu_extend_last and the Archimedean/separation properties
      -- Full proof requires the K&S separation argument machinery
      sorry -- TODO: Complete using separation set properties of δ
    ...
    have h_additive : ∀ (r s : Multi (k+1)),
      ...
      -- Key observations:
      -- 1. splitMulti (r + s) = (splitMulti r).1 + (splitMulti s).1 for first component
      --    and (splitMulti r).2 + (splitMulti s).2 for second component (pointwise addition)
      -- 2. R.add gives additivity on the old grid
      -- 3. δ multiplication is linear

      -- The full proof requires:
      -- (a) Showing Θ' value is independent of Classical.choose witness
      -- (b) Using R.add and iterate_op_add
      -- (c) Algebraic manipulation: (a + b) + (c + d) = (a + c) + (b + d)
      sorry -- TODO: Complete using witness independence + R.add + algebraic rearrangement

    -- Construct the MultiGridRep F'
    refine ⟨⟨Θ', h_strictMono, h_additive, h_norm⟩, trivial⟩
