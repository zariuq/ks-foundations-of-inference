...
compiled counterexample `Mettapedia/ProbabilityTheory/KnuthSkilling/RepresentationTheorem/Counterexamples/KSSeparationNotDerivable.lean`.

**Layer Structure** (avoiding circularity):
- **RepresentationTheorem/** assumes `[KSSeparation α]` and proves the representation theorem
...
This is the remaining hard step in the current `Separation/Derivation.lean` development. To avoid
