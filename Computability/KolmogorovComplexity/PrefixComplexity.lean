...
import Computability.KolmogorovComplexity.Prefix
...
We reuse the existing prefix-free machine framework from `Logic.SolomonoffPrior` and the
finite Kraft inequality from `Computability.KolmogorovComplexity.Prefix`.
-/
...
abbrev PrefixFreeMachine := Logic.SolomonoffPrior.PrefixFreeMachine
abbrev UniversalPFM := Logic.SolomonoffPrior.UniversalPFM
...
noncomputable abbrev prefixComplexity (U : PrefixFreeMachine) (x : BinString) : ℕ :=
  Logic.SolomonoffPrior.kolmogorovComplexity U x
...
noncomputable def shortestProgram (U : PrefixFreeMachine) [UniversalPFM U] (x : BinString) : BinString :=
  Classical.choose
    (Logic.SolomonoffPrior.exists_program_of_complexity (U := U) (x := x)
...
theorem invariance_Kpf (U V : PrefixFreeMachine) [UniversalPFM U] [UniversalPFM V] :
  ...
  obtain ⟨c, hc⟩ := Logic.SolomonoffPrior.invariance (U := U) (V := V)
  refine ⟨c, ?_⟩
  intro x
...
theorem sum_weightByKpf_le_one (U : PrefixFreeMachine) [UniversalPFM U] (s : Finset BinString) :
  ...
  have h_kraft :
    ...
    simpa [kraftSum, Logic.SolomonoffPrior.kraftSum] using
